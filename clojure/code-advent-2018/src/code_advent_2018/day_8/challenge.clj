            ...
            (if (zero? nodes-remaining)
                ...
                (let [[num-children num-metadata] (take 2 raw-tree)
                    ...
                    (if (zero? num-children)
                        (let [metadata (take num-metadata rest-raw-tree)]
                            (recur (drop num-metadata rest-raw-tree) (dec nodes-remaining) (conj nodes {:metadata metadata})))
                        (let [[children rest-raw-tree] (parse-tree rest-raw-tree num-children)
                              metadata (take num-metadata rest-raw-tree)]
                            (recur (drop num-metadata rest-raw-tree) (dec nodes-remaining) (conj nodes {:metadata metadata :children children})))))))))
